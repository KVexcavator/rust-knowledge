...
// версия узла криптовалюты
const NODE_VERSION: usize = 1;
// IP-адрес и номер порта центрального узла, к которому будет подключаться новый узел
pub const CENTERAL_NODE: &str = "127.0.0.1:2001";
//
pub const TRANSACTION_THRESHOLD: usize = 2;

// для хранения сетевых узлов, о которых знает этот узел
...
// для хранения ожидающих транзакций, которые ещё не были включены в блок
    ...
    pub fn run(&self, addr: &str) {
        ...
        for stream in listener.incoming() {
            ...
            thread::spawn(|| match stream {
                Ok(stream) => {
                  // пытается обслужить клиента, вызывая функцию serve с захваченным экземпляром блокчейна и потоком
                }
                Err(e) => {
                  // регистрируется сбой соединения
                }
            });
        }
    }
...
// типы операций 
// Tx - операции, связанные с транзакциями
// Block - действия, связанные с блоками внутри блокчейна
...
// передает запрос на определенные данные на указанный сетевой адрес
...
// уведомляет о конкретных элементах данных по указанному сетевому адресу
...
// передает блок на указанный сетевой адрес
...
// отправляет транзакцию на указанный сетевой адрес
...
// передавать информацию о версии
...
// передает запрос на блок данных
...
fn serve(blockchain: Blockchain, stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    for pkg in pkg_reader {
        ...
        match pkg {
            // десериализует блок и добавляет его в блокчейн
            Package::Block { addr_from, block } => {
                let block = Block::deserialize(block.as_slice());
                blockchain.add_block(&block);
                info!("Added block {}", block.get_hash());

                // Если есть блоки в пути, он отправляет запрос get_data для следующего блока.
                if GLOBAL_BLOCKS_IN_TRANSIT.len() > 0 {
                    ...
                    // Если блоков в пути больше нет, он переиндексирует набор UTXO блокчейна.
                    let utxo_set = UTXOSet::new(blockchain.clone());
                    utxo_set.reindex();
                }
            }
            // извлекает все хеши блоков из блокчейна и отправляет сообщение inv со списком хешей запрашивающему пиру
            Package::GetBlocks { addr_from } => {
                let blocks = blockchain.get_block_hashes();
                send_inv(addr_from.as_str(), OpType::Block, &blocks);
            }
            // извлекает запрошенный блок или транзакцию из блокчейна или глобального пула памяти и отправляет их обратно запрашивающему пиру
            Package::GetData {
                addr_from,
                op_type,
                id,
            ...
            // добавляет полученные блоки или транзакции в глобальные блоки в пути или пул памяти и при необходимости запрашивает недостающие блоки или транзакции через get_data
            Package::Inv {
                addr_from,
                op_type,
                items,
            ...
            // десериализует транзакцию и добавляет её в глобальный пул памяти.
            ...
            Package::Tx {
                ...
                //Если узел является майнером и пул памяти достиг определённого порога, он создаёт новый блок, содержащий транзакции из пула памяти, майнит его и транслирует новый блок другим узлам через inv
            ...
            // сравнивает высоты локального и удалённого блокчейнов и при необходимости отправляет сообщение get_blocks или version удалённому узлу. Он также добавляет удалённый узел в глобальный список известных узлов, если он ещё не присутствует
            Package::Version {
                addr_from,
                version,
                best_height,
    ...
    // возвращает результат, указывающий на успех или неудачу, и закрывает потоковое соединение после обработки всех входящих пакетов
    let _ = stream.shutdown(Shutdown::Both);
    Ok(())
}


// отправляет пакеты данных на указанный адрес сокета
